...
import { Menu, X, HeartPulse, UserCircle, Sparkles, Video, PhoneCall, MapPin, Globe } from 'lucide-react';
      ...
      <div className="bg-apollo-blue text-white py-2 px-4 sm:px-6 lg:px-8 hidden md:block">
        <div className="max-w-7xl mx-auto flex justify-between items-center text-[11px] font-bold uppercase tracking-wider">
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-2">
              <Globe className="w-3.5 h-3.5 text-apollo-red" />
              <span>International Patients</span>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <a href="#" className="hover:text-apollo-red transition-colors">Pay Online</a>
            <a href="#" className="hover:text-apollo-red transition-colors">GIRM Health</a>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden md:flex items-center space-x-3">
              {user ? (
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2 text-apollo-blue">
                ...
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className="text-[11px] font-bold uppercase text-apollo-blue hover:text-apollo-red px-4"
                  >
                    Login
                  </button>
                  <button
            ...
            <div className="md:hidden flex items-center">
              <button
                ...
                className="text-apollo-blue p-2"
              >
        ...
        <div className="md:hidden bg-white border-t border-slate-100 px-4 pt-2 pb-6 space-y-2 shadow-xl">
          {navLinks.map((link) => (
            <button
              key={link.name}
              onClick={() => handleLinkClick(link.path)}
              className="flex items-center space-x-3 w-full text-left px-4 py-3 text-sm font-bold uppercase text-apollo-blue hover:bg-apollo-grey hover:text-apollo-red rounded"
            >
              {link.icon && link.icon}
              <span>{link.name}</span>
            </button>
          ...
          <div className="pt-4 flex flex-col space-y-2 px-4 border-t border-slate-100">
            <button
              onClick={() => handleLinkClick('/enroll')}
              className="w-full bg-apollo-red text-white py-4 rounded font-black uppercase text-sm tracking-widest"
            >
              Book Appointment
            </button>
          </div>
